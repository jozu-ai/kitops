...
Get more information on unpack and filtering in the [CLI reference docs](/docs/cli/cli-reference/#kit-unpack).
...
If you have your AI/ML project artifacts in a directory structure already then the easiest way to get started is with [kit init](/docs/cli/cli-reference/#kit-init). From the root of the directory with the AI/ML artifacts you wish to pack in the ModelKit run:
...
You can learn more about the syntax, options, and flags in our [CLI docs](/docs/cli/cli-reference/#kit-init).
...
The `model` section can contain a single model, or you can create model dependencies with `model parts` which is covered in the [KitFile format documentation](/docs/kitfile/format/#model).
